...
export class NodeRepository {
    private filePath1 = join(process.cwd(), 'src', 'node', 'data.json');
    private filePath2 = join(process.cwd(), 'src', 'node', 'warehouse-stock.json');
    private filePath3 = join(process.cwd(), 'src', 'workers', 'drivers.json');
    private filePath4 = join(process.cwd(), 'src', 'node', 'pending-shipments.json');
    private agentsPath = join(process.cwd(), 'src', 'workers', 'agents.json');
    private agencyShipmentsPath = join(process.cwd(), 'src', 'node', 'agency-shipments.json');
    private shipmentsPath = join(process.cwd(), 'src', 'shipments', 'shipments.json');
    private configPath = join(process.cwd(), 'src', 'node', 'config.json');
    ...
    async updateProfile(id: string, name: string, email: string, phone: string) {
        const data = await this.readJson(this.filePath1);
        const cats = ['WAREHOUSE', 'TRANSIT_HUB', 'LOCAL_AGENCY', 'SUPERUSER', 'ADMIN', 'ADMIN_TEAMS'];
    ...
    async getWarehouseLatLng(warehouseId: string): Promise<{ lat: string | null, lng: string | null }> {
        const data = await this.readJson(this.filePath1);
        const wh = (data['WAREHOUSE'] || []).find((w: any) => w.id === warehouseId);
        return { lat: wh?.lat || null, lng: wh?.lng || null };
    }
    ...
    async getHubInventory(hubId: string) {
        ...
        let rtos = [];
        try {
            const rtoData = await this.readJson(join(process.cwd(), 'src', 'rto', 'rto.json'));
    ...
    async markDelivered(agencyId: string, trackingId: string) {
        ...
        if (shipment) {
            ...
            if (!shipment.updatedAt) shipment.updatedAt = [];
            shipment.updatedAt.push(new Date().toISOString());
            const agencyInfo = await this.getAgencyInfo(agencyId);
