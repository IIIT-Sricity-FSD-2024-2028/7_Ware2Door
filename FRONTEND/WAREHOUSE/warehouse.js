...
if (!session || !session.user) { window.location.href = "../AUTH/home.html"; }
let role = session.role;
let user = session.user;


async function w2dFetch(url, opts = {}) {
    const token = session?.token || '';
    const headers = Object.assign({ 'Content-Type': 'application/json', 'x-role': 'WAREHOUSE', 'Authorization': `Bearer ${token}` }, opts.headers || {});
