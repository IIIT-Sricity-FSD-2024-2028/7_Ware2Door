...
async function api(url, method = 'GET', body = null) {
  try {
    ...
    const opts = { method, credentials: 'include', headers: { 'Content-Type': 'application/json', 'x-role': 'LOCAL_AGENCY', 'Authorization': `Bearer ${token}` } };
    if (body) opts.body = JSON.stringify(body);
    const res = await fetch(API + url, opts);
    if (res.status === 401) { localStorage.removeItem('session'); window.location.href = '../AUTH/home.html'; return null; }
...
document.getElementById("ticket-escalate-btn").addEventListener("click", async () => {
  if (!currentViewingTicket) return;
  const status = normalizeTicketStatus(currentViewingTicket.status);
  if (status === "Resolved" || status === "Escalated") {
    showToast("This ticket is already " + status + ".", true);
    return;
  }
  const note = prompt("Escalation Note (describe why this needs central desk intervention):", "Customer unreachable after 3 delivery attempts.");
  if (note === null) return; // cancelled
  const t = ticketsList.find(x => x.ticketId === currentViewingTicket.ticketId);
  if (t) {
    const r = await api(`/tickets/${t.ticketId}/escalate`, 'PUT', { escalationNote: note || "Escalated by agency" });
    if (r && r.success) {
      t.status = "Escalated";
      currentViewingTicket.status = "Escalated";
      document.getElementById("detail-status-badge").textContent = "Escalated";
      document.getElementById("detail-status-badge").className = "ticket-status-badge status-escalated";
      const eb = document.getElementById("ticket-escalate-btn");
      eb.disabled = true; eb.style.opacity = "0.5";
      eb.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i> Escalated`;
      showToast(`✓ Ticket ${t.ticketId} escalated to Central Desk.`);
    } else {
      showToast("⚠ Failed to escalate ticket.", true);
    }
  }
});
...
document.getElementById("save-edit-modal").addEventListener("click", async () => {
  ...
  const res = await fetch(`${API}/auth/profile/${agencyId}`, {
      method: "POST",
      credentials: 'include',
      headers: { "Content-Type": "application/json", "x-role": "LOCAL_AGENCY", "Authorization": `Bearer ${session?.token || ''}` },
