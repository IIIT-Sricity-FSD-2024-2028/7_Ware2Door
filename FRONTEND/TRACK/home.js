...
async function fetchTrackingData(tracking_id, silent = false) {
    try {
        ...
        if (!res.ok) {
            if (!silent) alert(`No shipment found for Tracking ID "${tracking_id}". Please verify and try again.`);
            return null;
        }
        const data = await res.json();
        if (!data || data.error || !data.trackingId) {
            if (!silent) alert(data?.message || `No shipment found for Tracking ID "${tracking_id}". Please verify and try again.`);
            return null;
        }
        return data;
    } catch (e) {
        if (!silent) alert('Could not connect to tracking server. Please check your network connection.');
        return null;
    }
}
