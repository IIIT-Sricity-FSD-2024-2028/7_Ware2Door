...
_sessionRaw = JSON.parse(localStorage.getItem("session") || "null");
if (!_sessionRaw || !_sessionRaw.user) { window.location.href = "../AUTH/home.html"; }
_sessionRole = _sessionRaw.role || "TransitHub";
_sessionUser = _sessionRaw.user || {};


async function hubFetch(url, opts = {}) {
    const token = _sessionRaw?.token || '';
    const headers = Object.assign({ 'Content-Type': 'application/json', 'x-role': 'TRANSIT_HUB', 'Authorization': `Bearer ${token}` }, opts.headers || {});
    const options = Object.assign({ credentials: 'include' }, opts, { headers });
...
async function fetchHubInventory() {
    try {
        const res = await hubFetch(`${API_BASE}/hub/${_hubId}/inventory?t=${Date.now()}`);
...
async function fetchInbound() {
    try {
        const res = await hubFetch(`${API_BASE}/hub/${_hubId}/inbound?t=${Date.now()}`);
        const data = await res.json();
...
async function fetchOutbound() {
    try {
        const res = await hubFetch(`${API_BASE}/hub/${_hubId}/outbound?t=${Date.now()}`);
        const data = await res.json();
...
async function fetchScanHistory() {
    try {
        const res = await hubFetch(`${API_BASE}/hub/${_hubId}/scanHistory?t=${Date.now()}`);
        const data = await res.json();
...
async function fetchCapacity() {
    try {
        const res = await hubFetch(`${API_BASE}/hub/${_hubId}/capacity?t=${Date.now()}`);
        const data = await res.json();
...
async function fetchAgencies() {
    try {
        const res = await hubFetch(`${API_BASE}/system/agencies?t=${Date.now()}`);
        const data = await res.json();
...
async function apiInscan(trackingId) {
    const res = await hubFetch(`${API_BASE}/hub/${_hubId}/inscan`, {
        method: "POST",
...
async function apiOutscan(trackingId) {
    const res = await hubFetch(`${API_BASE}/hub/${_hubId}/outscan`, {
        method: "POST",
        body: JSON.stringify({ trackingId }),
    });
    return res.json();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    el("save-edit-modal").addEventListener("click", async () => {
        ...
        try {
            const res = await hubFetch(`${API_BASE}/auth/profile/${_hubId}`, {
                method: "POST",
    ...
    el("save-pwd-modal").addEventListener("click", async () => {
        ...
        try {
            const res = await hubFetch(`${API_BASE}/auth/password/${_hubId}`, {
                method: "POST",
